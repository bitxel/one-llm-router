...
| P0 | Models | API-key read-only model list/retrieve JSON request/response and provider errors; OAuth codex-lb-compatible `GET /v1/models` list facade maps to Codex model list. |
...
| P0 | `GET /v1/models` | Supported; direct forward. | Supported as codex-lb-compatible model list facade; map to the same Codex model-list source as `GET /backend-api/codex/models` and return OpenAI-compatible list shape. | JSON. | API-key model list success, OAuth facade model list success, provider/Codex mapping error. |
...
| `GET /v1/models` | Supported | Supported | API-key rows forward to `account.base_url + /v1/models`. OAuth rows return the Codex model list through a codex-lb-compatible OpenAI list facade and do not contact OpenAI Platform `/v1/models`. |
